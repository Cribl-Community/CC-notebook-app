...
- **``composio-client``** (``public/Examples/Composio_Python_SDK.ipynb``): use the **``composio-client``** wheel (pure Python), not the top-level **``composio``** package — the latter depends on **``openai`` → ``jiter``** (Rust; no Pyodide wheel). Pin **``composio-client==1.39.0``** to match the dependency line shipped with ``composio`` 0.13.x unless you intentionally upgrade both. Outbound calls target **``backend.composio.dev``** under ``/api/v3.1/``; declare that host in ``config/proxies.yml``. The bundled example uses **in-notebook placeholders** for the API key (no KV ``headers.inject``); do not commit real keys. **Install ``attrs`` before ``composio-client``** in the example notebook: PyPI also ships an unrelated package named ``attr``; Pyodide’s import hook can pull that by mistake, which breaks ``import attr`` with ``AttributeError: module 'attr' has no attribute 's'``.
