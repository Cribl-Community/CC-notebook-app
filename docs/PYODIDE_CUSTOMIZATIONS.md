...
- **``composio-client``** (``public/Examples/Composio_Python_SDK.ipynb``): use the **``composio-client``** wheel (pure Python, `py3-none-any`), not the top-level **``composio``** package — the latter depends on **``openai`` → ``jiter``** (Rust; no Pyodide wheel). Pin **``composio-client==1.39.0``** to match the dependency line shipped with ``composio`` 0.13.x; bump the pin deliberately when upgrading. Outbound calls target **``backend.composio.dev``** under ``/api/v3.1/``; that host is declared in ``config/proxies.yml`` with a ``/api/v3.1/`` prefix allowlist (single prefix covers all v3 sub-paths, which cannot be enumerated statically). **Auth via KV + proxy inject**: the Pyodide kernel bridge strips all custom request headers (see ``handleFetchRequest`` in ``PyodideKernel.ts``), so ``x-api-key`` set by ``composio-client`` never reaches the proxy. The fix is ``headers.inject: x-api-key: kv.composioApiKey`` in ``proxies.yml``; the notebook saves the user's key to ``kvstore/composioApiKey`` once via ``pyfetch``, after which the proxy injects it on every Composio request. **Install ``attrs`` before ``composio-client``**: Pyodide's auto-import hook calls ``micropip.install('attr')`` when it sees ``import attr``, installing an unrelated PyPI package named ``attr`` that lacks ``.s``; pre-installing ``attrs`` ensures the correct module wins ``sys.modules`` first.
- **``networkx``** (``public/Examples/Process_Lineage_Sigma_Hunt.ipynb``): ships in the Pyodide **lockfile** (``public/pyodide/pyodide-lock.json``) and loads on ``import networkx`` via the per-cell ``loadPackagesFromImports`` hook — **no ``micropip`` pin and no ``proxies.yml`` change** (the notebook's CSVs load through Cribl Search ``externaldata``, not Pyodide fetch). If a future ``PYODIDE_RELEASE`` drops ``networkx`` from the lockfile, add an explicit ``await micropip.install('networkx')`` in that notebook's Setup cell.
...
- Manual checks:
  ...
  - Composio example: `micropip` + `composio-client` against `backend.composio.dev` (save-to-KV cell + proxy header inject)
