...
- **`public/Examples/`** ships multiple curated `.ipynb` files; Welcome opens them via `fetch` + `ipynbTextToLoadPayload` into a new notebook tab.
...
- Example notebooks are maintained directly in `public/Examples/`; no generation script is used.

## Key files

- `src/notebook/tabWorkspace.ts`, `NotebookPage.tsx`, `Toolbar.tsx`, `WelcomePage.tsx`, `releaseNotes.ts`
- `public/Examples/*.ipynb`
