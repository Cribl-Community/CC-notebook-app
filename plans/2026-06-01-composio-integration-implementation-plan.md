...
| Path allowlist for proxy | `/api/v3.1/toolkits`, `/api/v3.1/tools` (prefix match; covers list + execute paths used by `composio-client` 1.39.x) |
...
- Add `backend.composio.dev` section with:
  - `paths.allowlist`: `/api/v3.1/toolkits`, `/api/v3.1/tools`
...
- Use strict proxy allowlists (`/api/v3.1/toolkits` and `/api/v3.1/tools` prefixes only). **Security trade-off:** API keys live in notebook cells at runtime; users must not commit filled-in values; consider org policy before promoting this pattern beyond examples.
...
| Question | Resolution |
|---|---|
| Exact Composio domain/path/auth contract | `backend.composio.dev`, `/api/v3.1/toolkits` + `/api/v3.1/tools` prefixes; auth via notebook placeholders → client sends API key (see Composio docs; typically `x-api-key`) |
